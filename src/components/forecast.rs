use crate::pages::home::DayData;
use crate::utils::date_utils::get_weekday_from_string;
use crate::utils::weather_utils::{get_weather_category, get_weather_icon};
use leptos::prelude::*;

#[component]
pub fn forecast(day_data: ReadSignal<Vec<DayData>>, is_light: ReadSignal<bool>) -> impl IntoView {
    let theme_suffix = if is_light.get() { "" } else { "-dark" };
    view! {
        <div class=format!("forecast-table-container{}", theme_suffix)>
            <table class=format!("forecast-table{}", theme_suffix) style="width: 100%; border-collapse: collapse; margin-top:20px;">
                <thead>
                    <tr>
